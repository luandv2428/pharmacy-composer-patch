...
/**
 * Controller responsible for initializing the SaaS project with Magento Services
 */
class Initialize extends AbstractAction
{
    /**
    ...
    public function execute() : ResultInterface
    {
        ...
        if (!empty($result['results'])) {
            try {
                $configs = [
                    ServicesConfig::CONFIG_PATH_PROJECT_ID => $result['results'][0]['projectId'],
                    ServicesConfig::CONFIG_PATH_PROJECT_NAME => $result['results'][0]['projectName'],
                    ServicesConfig::CONFIG_PATH_ENVIRONMENT_ID => $result['results'][0]['environmentId'],
                    ServicesConfig::CONFIG_PATH_ENVIRONMENT_NAME => $result['results'][0]['environmentName'],
                    ServicesConfig::CONFIG_PATH_ENVIRONMENT => $result['results'][0]['environmentType']
                ];
